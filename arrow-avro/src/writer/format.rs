...
/// block framing are handled by the top‑level `Writer`.
...
impl AvroFormat for AvroOcfFormat {
    fn start_stream<W: Write>(
        ...
        let schema_json = if self.encoder_options.impala_mode {
            maybe_rewrite_nullable_union_order(&avro_schema.json_string, true)?
        } else {
        ...
        // Metadata map (encoded as Avro map: one positive-length block then 0)
