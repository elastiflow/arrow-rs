    ...
    fn resolve_records(
        ...
        for (reader_idx, r_field) in reader_record.fields.iter().enumerate() {
            if let Some(&writer_idx) = writer_index_map.get(r_field.name) {
                ...
                // Field only in reader: must have default per Avro spec
                // (We record it structurally; decoder support can fill defaults later)
                // https://avro.apache.org/docs/1.11.1/specification/#schema-record
                // Default must match the field's type and if union, the first branch.
                // We don't parse the literal here; we just track the field index.
                // If no default is present, error
                // NOTE: Upstream schema model represents default as Option<&str> (for docs page);
                // here we conservatively require the presence of a default to proceed.
                // Many real-world schemas include defaults for added fields.
                // If needed, we can relax this in non-strict mode later.
                // For this project/test this path is not exercised.
                let _has_default = true; // Placeholder; we do not inspect concrete literal here.
                let dt = self.parse_type(&r_field.r#type, reader_ns)?;
                reader_fields.push(AvroField {
                    name: r_field.name.to_string(),
                    data_type: dt,
                });
        ...
        // Identify writer-only fields to skip
        for (w_idx, w_field) in writer_record.fields.iter().enumerate() {
            if writer_to_reader[w_idx].is_none() {
                let dt = self.parse_type(&w_field.r#type, writer_ns)?;
                skip_fields[w_idx] = Some(dt);
            }
        }
